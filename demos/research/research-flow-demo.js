...
import logger from "../project-manager/src/core/utils/logger.js";
import {
  getBestAvailableAIModel,
  getPerplexityClientForMCP,
  getModelConfig,
} from "../project-manager/src/core/utils/ai-client-utils.js";
    ...
    try {
      ...
      if (this.queryType === "research") {
        ...
        try {
          ...
          logger.warn(
            "Perplexity not available, falling back to other providers:",
      ...
      switch (provider) {
        case "perplexity":
          ...
          try {
            ...
            const messages = [
              {
                role: "system",
                content:
                  "You are a helpful research assistant. Provide detailed, accurate information with citations where possible.",
              },
              {
                role: "user",
            ...
            if (
              ...
              logger.debug(
                "Content extracted successfully (first 100 chars):",
              ...
              throw new Error(
                "Unexpected response format from Perplexity API: missing content",
              );
            }
          } catch (perplexityError) {
        ...
        case "anthropic":
          ...
          if (response && response.content && response.content[0]) {
            ...
            throw new Error(
              "Unexpected response format from Anthropic API: missing content",
            );
          }
          break;
        ...
        case "openai":
          ...
          if (
            ...
            throw new Error(
              "Unexpected response format from OpenAI API: missing content",
            );
          }
          break;
          ...
          try {
            ...
            if (geminiResult && geminiResult.response) {
              ...
              if (response && typeof response.text === "function") {
                ...
                throw new Error(
                  "Unexpected response format from Gemini API: response.text is not a function",
                );
              }
            } else {
              throw new Error(
                "Unexpected response format from Gemini API: missing response property",
              );
            }
          } catch (geminiError) {
...
class InformationExtractorNode extends AsyncNode {
  ...
  async exec(data) {
    ...
    try {
      ...
      logger.info(
        `Extracted ${finalKeyPoints.length} key points and ${codeBlocks.length} code blocks`,
      );
...
class ConfidenceScorerNode extends AsyncNode {
  ...
  async exec(data) {
    ...
    try {
      ...
      if (extractedInfo.keyPoints && extractedInfo.keyPoints.length >= 3)
        score += 0.1;
      if (extractedInfo.keyPoints && extractedInfo.keyPoints.length >= 5)
        score += 0.1;

      // Adjust score based on code examples
      if (extractedInfo.codeBlocks && extractedInfo.codeBlocks.length >= 1)
        score += 0.1;
      if (extractedInfo.codeBlocks && extractedInfo.codeBlocks.length >= 3)
        score += 0.1;
...
function displayResults(data) {
  try {
    ...
    if (result.provider) {
      console.log(
        `PROVIDER: ${result.provider.name || "unknown"} (${result.provider.model || "unknown model"})`,
      );
    } else {
...
async function main() {
  try {
    ...
    const query =
      process.argv[2] ||
      "Best practices for implementing a flow-based architecture in JavaScript";
