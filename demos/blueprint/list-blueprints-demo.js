...
import { listBlueprints } from "../project-manager/src/mvp/simple-blueprint-direct.js";
...
async function runListBlueprintsDemo() {
  try {
    ...
    if (result.blueprints.length === 0) {
      ...
      console.log(
        'npm run mvp:create-blueprint "Project Name" "Description" "Requirements"',
      );
    } else {
      ...
      result.blueprints.forEach((blueprint, index) => {
        ...
        console.log(
          `   ${chalk.dim("Created:")} ${new Date(blueprint.createdAt).toLocaleString()}`,
        );
