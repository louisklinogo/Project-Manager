...
async function runDemo() {
  ...
  try {
    ...
    const revalidationResult = validator.validateDependencies(resolutionResult.tasks);
    console.log(`Validation result: ${revalidationResult.valid ? 'Valid' : 'Invalid'}`);
    ...
    if (optimizationResult.tasks.length > 0) {
      analyzeDependencyImpact(optimizationResult.tasks, optimizationResult.tasks[0].id);
    }
