/**
 * tools/utils.js
 * Utility functions for Project Manager CLI integration
 */
...
function getProjectRoot(projectRootRaw, log) {
  ...
  log.warn(
    `No project-manager project detected in current directory. Using ${currentDir} as project root.`,
  );
  log.warn(
...
/**
 * Executes a project-manager CLI command synchronously.
 ...
 */
function executeProject-ManagerCommand(
  command,
  log,
  args = [],
  projectRootRaw = null,
  ...
  try {
    ...
    log.info(
      `Executing project-manager ${command} with args: ${JSON.stringify(
        args,
      )} in directory: ${cwd}`,
    );
    ...
    // Execute the command using the global project-manager CLI or local script
    // Try the global CLI first
    let result = spawnSync("project-manager", fullArgs, spawnOptions);

    // If global CLI is not available, try fallback to the local script
    if (result.error && result.error.code === "ENOENT") {
      log.info("Global project-manager not found, falling back to local script");
    ...
    log.error(`Error executing project-manager command: ${error.message}`);
    return {
      success: false,
      error: error.message,
    };
