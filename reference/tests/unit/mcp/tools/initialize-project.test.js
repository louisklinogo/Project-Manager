...
const registerInitializeProjectTool = (server) => {
  server.addTool({
    ...
    description:
      "Initializes a new Project Manager project structure in the current working directory by running 'project-manager init'.",
    parameters: mockZod,
    execute: async (args, { log }) => {
      try {
        ...
        let command = "npx project-manager init";
        const cliArgs = [];
        if (args.projectName) {
...
describe("Initialize Project MCP Tool", () => {
  ...
  test("registers the tool with correct name and parameters", () => {
    ...
    expect(toolDefinition.description).toContain(
      "Initializes a new Project Manager project",
    );
  ...
  test("constructs command with proper arguments", async () => {
    ...
    // Check that the command includes npx project-manager init
    expect(command).toContain("npx project-manager init");
