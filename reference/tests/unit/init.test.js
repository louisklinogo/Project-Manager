...
jest.mock("figlet", () => ({
  default: {
    textSync: jest.fn(() => "Project Manager"),
  },
}));
...
describe("Windsurf Rules File Handling", () => {
      ...
      if (filename === ".windsurfrules") {
        if (fs.existsSync(targetPath)) {
          ...
          const updatedContent =
            ...
            "\n\n# Added by Claude Project Manager - Development Workflow Rules\n\n" +
            "New content";
  ...
  test("appends content to existing .windsurfrules", () => {
    ...
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      targetPath,
      expect.stringContaining("Added by Claude Project Manager"),
    );
  });
...
describe("MCP Configuration Handling", () => {
  ...
  function mockSetupMCPConfiguration(targetDir, projectName) {
    ...
    const newMCPServer = {
      "project-manager-ai": {
        command: "npx",
        args: ["project-manager-ai", "mcp-server"],
      },
    };
    ...
    if (fs.existsSync(mcpJsonPath)) {
      try {
        ...
        if (!mcpConfig.mcpServers["project-manager-ai"]) {
          mcpConfig.mcpServers["project-manager-ai"] =
            newMCPServer["project-manager-ai"];
        }
  ...
  test("creates mcp.json when it does not exist", () => {
    ...
    expect(fs.writeFileSync).toHaveBeenCalledWith(
      mcpJsonPath,
      expect.stringContaining("project-manager-ai"),
    );
  ...
  test("does not modify existing server configuration if it already exists", () => {
    ...
    // Return JSON that already has project-manager-ai
    fs.readFileSync.mockImplementation((filePath) => {
      if (filePath.toString().includes("mcp.json")) {
        return JSON.stringify({
          mcpServers: {
    ...
    // Assert
    // Verify the written data contains the original project-manager configuration
    const dataWritten = JSON.parse(writeFileSyncSpy.mock.calls[0][1]);
    expect(dataWritten.mcpServers["project-manager-ai"].command).toBe("custom");
    expect(dataWritten.mcpServers["project-manager-ai"].args).toContain(
      "custom-args",
    );
  });
