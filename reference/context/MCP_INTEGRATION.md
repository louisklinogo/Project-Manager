# Project Manager MCP Integration

This document outlines how Project Manager CLI functionality is integrated with MCP (Master Control Program) architecture to provide both CLI and programmatic API access to features.
...
3. **Project Manager Core** - In `mcp-server/src/core/project-manager-core.js` provides direct function imports
...
3. **Add to project-manager-core.js** to make it available for direct import
...
### Project Manager Core Update

```javascript
// In mcp-server/src/core/project-manager-core.js
  ...
  server.addTool({
    ...
    execute: async (args, { log }) => {
      try {
        ...
        // Execute the command
        const result = await executeProject-ManagerCommand(
          "new-feature",
          log,
          cmdArgs,
          projectRoot,
...
export function registerProject-ManagerTools(server) {
