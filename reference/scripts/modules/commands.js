...
function registerCommands(programInstance) {
  ...
  programInstance.on("option:unknown", function (unknownOption) {
    ...
    console.error(
      chalk.yellow(
        `Run 'project-manager ${commandName} --help' to see available options`,
      ),
    );
    process.exit(1);
  });
        ...
        console.log(
          boxen(
            chalk.white.bold("Parse PRD Help") +
              ...
              chalk.cyan("Usage:") +
              "\n" +
              `  project-manager parse-prd <prd-file.txt> [options]\n\n` +
              chalk.cyan("Options:") +
              "\n" +
              ...
              chalk.cyan("Example:") +
              "\n" +
              "  project-manager parse-prd requirements.txt --num-tasks 15\n" +
              "  project-manager parse-prd --input=requirements.txt\n" +
              "  project-manager parse-prd --force\n" +
              "  project-manager parse-prd requirements_v2.txt --append\n\n" +
  ...
  programInstance
    ...
    .action(async (options) => {
      ...
      if (
        ...
        console.log(
          `  project-manager update --from=${fromId} --prompt="Your prompt here"`,
        );
        console.log(
          chalk.yellow(
        ...
        console.log(
          `  project-manager update-task --id=<id> --prompt="Your prompt here"`,
        );
        process.exit(1);
      }
    ...
    .action(async (options) => {
      try {
        ...
        if (!options.id) {
          ...
          console.log(
            chalk.yellow(
              'Usage example: project-manager update-task --id=23 --prompt="Update with new information"',
            ),
          );
          process.exit(1);
        }
        ...
        if (!fs.existsSync(tasksPath)) {
          ...
          if (tasksPath === "tasks/tasks.json") {
            console.log(
              chalk.yellow(
                "Hint: Run project-manager init or project-manager parse-prd to create tasks.json first",
              ),
            );
          } else {
            console.log(
        ...
        if (
          ...
          console.log(chalk.yellow("\nTo fix this issue:"));
          console.log(
            "  1. Run project-manager list to see all available task IDs",
          );
    ...
    .action(async (options) => {
      try {
        ...
        if (!options.id) {
          ...
          console.log(
            chalk.yellow(
              'Usage example: project-manager update-subtask --id=5.2 --prompt="Add more details about the API endpoint"',
            ),
          );
          process.exit(1);
        }
  ...
  programInstance
    ...
    .action(async (options) => {
      ...
      if (options.all) {
        console.log(
          chalk.blue(
            `Expanding all tasks with ${numSubtasks} subtasks each...`,
          ),
        );
        if (useResearch) {
          console.log(
          ...
          console.log(
            boxen(
              chalk.white.bold(
                ...
                "\n" +
                chalk.cyan(
                  `1. Run ${chalk.yellow(`project-manager show ${parentId}`)} to see the parent task with all subtasks`,
                ) +
                "\n" +
                chalk.cyan(
                  `2. Run ${chalk.yellow(`project-manager set-status --id=${parentId}.${subtask.id} --status=in-progress`)} to start working on it`,
                ),
              {
                padding: 1,
                borderColor: "green",
          ...
          console.log(
            boxen(
              chalk.white.bold("Usage Examples:") +
                ...
                "\n" +
                chalk.yellow(
                  `  project-manager add-subtask --parent=5 --task-id=8`,
                ) +
                "\n\n" +
                chalk.white("Create new subtask:") +
                "\n" +
                chalk.yellow(
                  `  project-manager add-subtask --parent=5 --title="Implement login UI" --description="Create the login form"`,
                ) +
                "\n\n",
              { padding: 1, borderColor: "blue", borderStyle: "round" },
            ),
    ...
    console.log(
      boxen(
        chalk.white.bold("Add Subtask Command Help") +
          ...
          chalk.cyan("Usage:") +
          "\n" +
          `  project-manager add-subtask --parent=<id> [options]\n\n` +
          chalk.cyan("Options:") +
          "\n" +
          ...
          "\n" +
          "  project-manager add-subtask --parent=5 --task-id=8\n" +
          '  project-manager add-subtask -p 5 -t "Implement login UI" -d "Create the login form"',
        { padding: 1, borderColor: "blue", borderStyle: "round" },
      ),
    );
  }
            ...
            console.log(
              boxen(
                chalk.white.bold(
                  ...
                  "\n" +
                  chalk.cyan(
                    `1. Run ${chalk.yellow(`project-manager show ${result.id}`)} to see details of the new task`,
                  ) +
                  "\n" +
                  chalk.cyan(
                    `2. Run ${chalk.yellow(`project-manager set-status --id=${result.id} --status=in-progress`)} to start working on it`,
                  ),
                {
                  padding: 1,
                  borderColor: "green",
    ...
    console.log(
      boxen(
        chalk.white.bold("Remove Subtask Command Help") +
          ...
          chalk.cyan("Usage:") +
          "\n" +
          `  project-manager remove-subtask --id=<parentId.subtaskId> [options]\n\n` +
          chalk.cyan("Options:") +
          "\n" +
          ...
          chalk.cyan("Examples:") +
          "\n" +
          "  project-manager remove-subtask --id=5.2\n" +
          "  project-manager remove-subtask --id=5.2,6.3,7.1\n" +
          "  project-manager remove-subtask --id=5.2 --convert",
        { padding: 1, borderColor: "blue", borderStyle: "round" },
      ),
    );
  }
  ...
  programInstance
    ...
    .action(async (options) => {
      ...
      if (!taskIds) {
        ...
        console.error(
          chalk.yellow("Usage: project-manager remove-task --id=<taskId>"),
        );
        process.exit(1);
      }
  ...
  programInstance
    .command("init")
    .description("Initialize a new project with Project Manager structure")
    ...
    .option("--aliases", "Add shell aliases (tm, project-manager)")
...
/**
 * Check for newer version of project-manager-ai
...
function displayUpgradeNotification(currentVersion, latestVersion) {
  const message = boxen(
    `${chalk.blue.bold("Update Available!")} ${chalk.dim(currentVersion)} → ${chalk.green(latestVersion)}\n\n` +
      `Run ${chalk.cyan("npm i project-manager-ai@latest -g")} to update to the latest version with new features and bug fixes.`,
    {
      padding: 1,
