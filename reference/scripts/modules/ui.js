/**
 * ui.js
 * User interface functions for the Project Manager CLI
 */
...
function displayBanner() {
  console.clear();
  const bannerText = figlet.textSync("Project Manager", {
    font: "Standard",
...
function displayHelp() {
  ...
  console.log(
    boxen(chalk.white.bold("Project Manager CLI"), {
      padding: 1,
      borderColor: "blue",
      borderStyle: "round",
    ...
    console.log(
      boxen(
        chalk.yellow("No subtasks found. Consider breaking down this task:") +
          "\n" +
          chalk.white(
            `Run: ${chalk.cyan(`project-manager expand --id=${nextTask.id}`)}`,
          ),
        {
          padding: { top: 0, bottom: 0, left: 1, right: 1 },
          borderColor: "yellow",
  ...
  console.log(
    boxen(
      chalk.white.bold("Suggested Actions:") +
        "\n" +
        `${chalk.cyan("1.")} Mark as in-progress: ${chalk.yellow(`project-manager set-status --id=${nextTask.id} --status=in-progress`)}\n` +
        `${chalk.cyan("2.")} Mark as done when completed: ${chalk.yellow(`project-manager set-status --id=${nextTask.id} --status=done`)}\n` +
        (nextTask.subtasks && nextTask.subtasks.length > 0
          ? `${chalk.cyan("3.")} Update subtask status: ${chalk.yellow(`project-manager set-status --id=${nextTask.id}.1 --status=done`)}`
          : `${chalk.cyan("3.")} Break down into subtasks: ${chalk.yellow(`project-manager expand --id=${nextTask.id}`)}`),
      {
        padding: { top: 0, bottom: 0, left: 1, right: 1 },
        borderColor: "green",
        borderStyle: "round",
...
async function displayTaskById(tasksPath, taskId) {
  ...
  if (task.isSubtask || task.parentTask) {
    ...
    console.log(
      boxen(
        chalk.white.bold("Suggested Actions:") +
          "\n" +
          `${chalk.cyan("1.")} Mark as in-progress: ${chalk.yellow(`project-manager set-status --id=${task.parentTask.id}.${task.id} --status=in-progress`)}\n` +
          `${chalk.cyan("2.")} Mark as done when completed: ${chalk.yellow(`project-manager set-status --id=${task.parentTask.id}.${task.id} --status=done`)}\n` +
          `${chalk.cyan("3.")} View parent task: ${chalk.yellow(`project-manager show --id=${task.parentTask.id}`)}`,
        {
          padding: { top: 0, bottom: 0, left: 1, right: 1 },
          borderColor: "green",
          borderStyle: "round",
    ...
    console.log(
      boxen(
        chalk.yellow("No subtasks found. Consider breaking down this task:") +
          "\n" +
          chalk.white(
            `Run: ${chalk.cyan(`project-manager expand --id=${task.id}`)}`,
          ),
        {
          padding: { top: 0, bottom: 0, left: 1, right: 1 },
          borderColor: "yellow",
  ...
  console.log(
    boxen(
      chalk.white.bold("Suggested Actions:") +
        "\n" +
        `${chalk.cyan("1.")} Mark as in-progress: ${chalk.yellow(`project-manager set-status --id=${task.id} --status=in-progress`)}\n` +
        `${chalk.cyan("2.")} Mark as done when completed: ${chalk.yellow(`project-manager set-status --id=${task.id} --status=done`)}\n` +
        (task.subtasks && task.subtasks.length > 0
          ? `${chalk.cyan("3.")} Update subtask status: ${chalk.yellow(`project-manager set-status --id=${task.id}.1 --status=done`)}`
          : `${chalk.cyan("3.")} Break down into subtasks: ${chalk.yellow(`project-manager expand --id=${task.id}`)}`),
      {
        padding: { top: 0, bottom: 0, left: 1, right: 1 },
        borderColor: "green",
        borderStyle: "round",
...
async function displayComplexityReport(reportPath) {
  ...
  tasksNeedingExpansion.forEach((task) => {
    const expansionCommand = `project-manager expand --id=${task.taskId} --num=${task.recommendedSubtasks}${task.expansionPrompt ? ` --prompt="${task.expansionPrompt}"` : ""}`;
  ...
  console.log(
    boxen(
      chalk.white.bold("Suggested Actions:") +
        "\n\n" +
        `${chalk.cyan("1.")} Expand all complex tasks: ${chalk.yellow(`project-manager expand --all`)}\n` +
        `${chalk.cyan("2.")} Expand a specific task: ${chalk.yellow(`project-manager expand --id=<id>`)}\n` +
        `${chalk.cyan("3.")} Regenerate with research: ${chalk.yellow(`project-manager analyze-complexity --research`)}`,
      {
        padding: 1,
        borderColor: "cyan",
        borderStyle: "round",
