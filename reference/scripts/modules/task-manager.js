/**
 * task-manager.js
 * Task management functions for the Project Manager CLI
 */
      ...
      console.log(
        boxen(
          chalk.white.bold("Next Steps:") +
            "\n\n" +
            `${chalk.cyan("1.")} Run ${chalk.yellow("project-manager list")} to view all tasks\n` +
            `${chalk.cyan("2.")} Run ${chalk.yellow("project-manager expand --id=<id>")} to break down a task into subtasks`,
          {
            padding: 1,
            borderColor: "cyan",
            borderStyle: "round",
...
Return only the updated task as a valid JSON object.`,
    ...
    if (outputFormat === "text") {
      ...
      if (error.message?.includes("ANTHROPIC_API_KEY")) {
        ...
        console.log(
          '  2. Or run without the research flag: project-manager update --from=<id> --prompt="..."',
        );
...
Return only the updated task as a valid JSON object.`,
    ...
    if (outputFormat === "text") {
      ...
      if (error.message.includes("ANTHROPIC_API_KEY")) {
        ...
        console.log(
          '  2. Or run without the research flag: project-manager update-task --id=<id> --prompt="..."',
        );
      } else if (
        ...
        console.log(chalk.yellow("\nTo fix this issue:"));
        console.log("  1. Run project-manager list to see all available task IDs");
    ...
    if (
      ...
      if (
        ...
        if (showUi) {
          ...
          console.log(
            chalk.yellow(
              `Consider updating the parent task status with: project-manager set-status --id=${parentId} --status=done`,
            ),
          );
        }
      }
      ...
      console.log(
        boxen(
          chalk
            ...
            subtasksSection +
            "\n\n" +
            `${chalk.cyan("Start working:")} ${chalk.yellow(`project-manager set-status --id=${nextTask.id} --status=in-progress`)}\n` +
            `${chalk.cyan("View details:")} ${chalk.yellow(`project-manager show ${nextTask.id}`)}`,
          {
    ...
    console.log(
      boxen(
        chalk.white.bold("Suggested Next Steps:") +
          "\n\n" +
          `${chalk.cyan("1.")} Run ${chalk.yellow("project-manager next")} to see what to work on next\n` +
          `${chalk.cyan("2.")} Run ${chalk.yellow("project-manager expand --id=<id>")} to break down a task into subtasks\n` +
          `${chalk.cyan("3.")} Run ${chalk.yellow("project-manager set-status --id=<id> --status=done")} to mark a task as complete`,
        {
          padding: 1,
          borderColor: "gray",
          borderStyle: "round",
  ...
  try {
    ...
    for (const task of tasksToExpand) {
      ...
      try {
        ...
        if (
          ...
          report(
            `Suggestion: Run 'project-manager update-task --id=${task.id} --prompt="Generate subtasks for this task"' to manually create subtasks.`,
            "info",
          );
          expansionErrors++;
        }
    ...
    if (outputFormat === "text") {
      ...
      if (expandedCount > 0) {
        ...
        console.log(
          chalk.cyan(
            `1. Run ${chalk.yellow("project-manager list --with-subtasks")} to see all tasks with their subtasks`,
          ),
        );
        console.log(
          chalk.cyan(
            `2. Run ${chalk.yellow("project-manager next")} to find the next task to work on`,
          ),
        );
        console.log(
          chalk.cyan(
            `3. Run ${chalk.yellow("project-manager set-status --id=<taskId> --status=in-progress")} to start working on a task`,
          ),
        );
      }
    }
  ...
  if (clearedCount > 0) {
    ...
    console.log(
      boxen(
        chalk.white.bold("Next Steps:") +
          "\n\n" +
          `${chalk.cyan("1.")} Run ${chalk.yellow("project-manager expand --id=<id>")} to generate new subtasks\n` +
          `${chalk.cyan("2.")} Run ${chalk.yellow("project-manager list --with-subtasks")} to verify changes`,
        {
          padding: 1,
          borderColor: "cyan",
          borderStyle: "round",
      ...
      console.log(
        boxen(
          chalk.white.bold(`Task ${newTaskId} Created Successfully`) +
            ...
            "\n" +
            chalk.cyan(
              `1. Run ${chalk.yellow(`project-manager show ${newTaskId}`)} to see complete task details`,
            ) +
            "\n" +
            chalk.cyan(
              `2. Run ${chalk.yellow(`project-manager set-status --id=${newTaskId} --status=in-progress`)} to start working on it`,
            ) +
            "\n" +
            chalk.cyan(
              `3. Run ${chalk.yellow(`project-manager expand --id=${newTaskId}`)} to break it down into subtasks`,
            ),
          { padding: 1, borderColor: "green", borderStyle: "round" },
        ),
      );
...
DO NOT include any text before or after the JSON array. No explanations, no markdown formatting.`;
        ...
        if (outputFormat === "text") {
          ...
          console.log(
            boxen(
              chalk.white.bold("Suggested Next Steps:") +
                "\n\n" +
                `${chalk.cyan("1.")} Run ${chalk.yellow("project-manager complexity-report")} to review detailed findings\n` +
                `${chalk.cyan("2.")} Run ${chalk.yellow("project-manager expand --id=<id>")} to break down complex tasks\n` +
                `${chalk.cyan("3.")} Run ${chalk.yellow("project-manager expand --all")} to expand all pending tasks based on complexity`,
              {
                padding: 1,
                borderColor: "cyan",
                borderStyle: "round",
    ...
    if (outputFormat === "text") {
      ...
      if (error.message.includes("ANTHROPIC_API_KEY")) {
        ...
        console.log(
          "  2. Or run without the research flag: project-manager analyze-complexity",
        );
      }
...
Provide concrete examples, code snippets, or implementation details when relevant.`;
    ...
    if (outputFormat === "text") {
      ...
      if (error.message?.includes("ANTHROPIC_API_KEY")) {
        ...
        console.log(
          '  2. Or run without the research flag: project-manager update-subtask --id=<id> --prompt=\"...\"',
        );
      } else if (error.message?.includes("overloaded")) {
        ...
        console.log(chalk.yellow("\nTo fix this issue:"));
        console.log(
          "  1. Run project-manager list --with-subtasks to see all available subtask IDs",
        );
        console.log(
  ...
  try {
    ...
    try {
      ...
      return {
        ...
        suggestion:
          "Use 'project-manager update-task --id=<id> --prompt=\"Generate subtasks for this task\"' to manually create subtasks.",
      };
    }
  } catch (error) {
    if (mcpLog) {
