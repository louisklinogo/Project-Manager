...
function displayBanner() {
  ...
  console.clear();
  const bannerText = figlet.textSync("Project Manager AI", {
    font: "Standard",
...
function addShellAliases() {
  ...
  try {
    ...
    if (configContent.includes("alias tm='project-manager'")) {
      log("info", "Project Manager aliases already exist in shell config.");
      return true;
    }

    // Add aliases to the shell config file
    const aliasBlock = `
# Project Manager aliases added on ${new Date().toLocaleDateString()}
alias tm='project-manager'
alias project-manager='project-manager'
`;

    fs.appendFileSync(shellConfigFile, aliasBlock);
    log("success", `Added Project Manager aliases to ${shellConfigFile}`);
    log(
      "info",
...
function copyTemplateFile(templateName, targetPath, replacements = {}) {
  ...
  switch (templateName) {
    ...
    case "self_improve.mdc":
      ...
      break;
    case "README-project-manager.md":
      sourcePath = path.join(__dirname, "..", "README-project-manager.md");
      break;
    case "windsurfrules":
  ...
  if (fs.existsSync(targetPath)) {
    ...
    if (filename === ".gitignore") {
      ...
      if (newLines.length > 0) {
        ...
        const updatedContent =
          ...
          "\n\n# Added by Claude Project Manager\n" +
          newLines.join("\n");
    ...
    if (filename === ".windsurfrules") {
      ...
      const updatedContent =
        existingContent.trim() +
        "\n\n# Added by Project Manager - Development Workflow Rules\n\n" +
        content;
      fs.writeFileSync(targetPath, updatedContent);
    ...
    if (filename === "README-project-manager.md") {
      log("info", `${targetPath} already exists`);
      // Create a separate README file specifically for this project
      const taskMasterReadmePath = path.join(
        path.dirname(targetPath),
        "README-project-manager.md",
      );
      fs.writeFileSync(taskMasterReadmePath, content);
      log(
        "success",
        `Created ${taskMasterReadmePath} (preserved original README-project-manager.md)`,
      );
      return;
    }
...
async function initializeProject(options = {}) {
  ...
  if (skipPrompts) {
    ...
    if (dryRun) {
      ...
      log("info", "Would initialize Project Manager project");
      log("info", "Would create/update necessary project files");
      if (addAliases) {
        log("info", "Would add shell aliases for project-manager");
      }
      return {
        dryRun: true,
      };
    ...
    try {
      ...
      const addAliasesInput = await promptQuestion(
        rl,
        chalk.cyan(
          'Add shell aliases for project-manager? This lets you type "tm" instead of "project-manager" (Y/n): ',
        ),
      );
      ...
      console.log(
        chalk.blue(
          'Add shell aliases (so you can use "tm" instead of "project-manager"):',
        ),
...
function createProjectStructure(addAliases) {
  ...
  // Copy project-manager.mdc
  copyTemplateFile(
    "project-manager.mdc",
    path.join(targetDir, ".cursor", "rules", "project-manager.mdc"),
  );
  ...
  copyTemplateFile(
    "README-project-manager.md",
    path.join(targetDir, "README-project-manager.md"),
    replacements,
  );
    ...
    console.log(
      boxen(
        chalk.cyan.bold("Things you can now do:") +
          ...
          chalk.cyan("project-manager parse-prd <your-prd-file.txt>") +
          "\n" +
          chalk.white("4. ") +
...
function setupMCPConfiguration(targetDir) {
  ...
  if (fs.existsSync(mcpJsonPath)) {
    log(
      "info",
      "MCP configuration file already exists, checking for existing project-manager-mcp...",
    );
    try {
      ...
      // Check if any existing server configuration already has project-manager-mcp in its args
      const hasMCPString = Object.values(mcpConfig.mcpServers).some(
        (server) =>
          server.args &&
          server.args.some(
            (arg) => typeof arg === "string" && arg.includes("project-manager-mcp"),
          ),
      );
      ...
      if (hasMCPString) {
        log(
          "info",
          "Found existing project-manager-mcp configuration in mcp.json, leaving untouched",
        );
        return; // Exit early, don't modify the existing configuration
      }

      // Add the project-manager-ai server if it doesn't exist
      if (!mcpConfig.mcpServers["project-manager-ai"]) {
        mcpConfig.mcpServers["project-manager-ai"] = newMCPServer["project-manager-ai"];
        log(
          "info",
          "Added project-manager-ai server to existing MCP configuration",
        );
      } else {
        log("info", "project-manager-ai server already configured in mcp.json");
      }
  ...
  log("info", "MCP server will use the installed project-manager-ai package");
}
