...
2. **CLI Commands**  
   ...
   Run `project-manager --help` or `node scripts/dev.js --help` to see detailed usage information.
...
project-manager list --status=pending
...
project-manager list --status=pending --with-subtasks
```
...
project-manager update --from=4 --prompt="Refactor tasks from ID 4 onward to use Express instead of Fastify"

# Update all tasks (default from=1)
project-manager update --prompt="Add authentication to all relevant tasks"

# Specify a different tasks file
project-manager update --file=custom-tasks.json --from=5 --prompt="Change database from MongoDB to PostgreSQL"
```
...
project-manager set-status --id=4 --status=pending

# Mark a specific subtask as done
project-manager set-status --id=3.1 --status=done

# Mark multiple tasks at once
project-manager set-status --id=1,2,3 --status=done
```
...
project-manager expand --id=3

# Expand a specific task with 5 subtasks
project-manager expand --id=3 --num=5

# Expand a task with additional context
project-manager expand --id=3 --prompt="Focus on security aspects"
...
project-manager expand --id=3 --research
...
project-manager clear-subtasks --id=3
...
# Specify a different tasks file
project-manager validate-dependencies --file=custom-tasks.json
```
...
# Specify a different tasks file
project-manager fix-dependencies --file=custom-tasks.json
```
...
project-manager analyze-complexity --output=custom-report.json
  ...
  "complexityAnalysis": [
    {
      ...
      "expansionCommand": "project-manager expand --id=8 --num=6 --prompt=\"Create subtasks...\" --research"
    }
...
# Specify a different tasks file
project-manager next --file=custom-tasks.json
```
...
project-manager show 1

# Alternative syntax with --id option
project-manager show --id=1

# Show details for a subtask
project-manager show --id=1.2

# Specify a different tasks file
project-manager show 3 --file=custom-tasks.json
```
