...
function createDevScriptAction(commandName) {
  return (options, cmd) => {
    ...
    if (camelCaseFlags.length > 0) {
      ...
      console.error(
        "\nExample: project-manager parse-prd --num-tasks=5 instead of --numTasks=5\n",
      );
      process.exit(1);
    }
...
program
  ...
  .description("Claude Project Manager CLI")
  .version(version)
...
process.on("uncaughtException", (err) => {
  ...
  if (err.code === "commander.unknownOption") {
    ...
    console.error(
      chalk.yellow(
        `Run 'project-manager ${command} --help' to see available options for this command`,
      ),
    );
    process.exit(1);
  }
  ...
  if (err.code === "commander.unknownCommand") {
    ...
    console.error(
      chalk.yellow(`Run 'project-manager --help' to see available commands`),
    );
    process.exit(1);
  }
...
// Show help if no command was provided (just 'project-manager' with no args)
